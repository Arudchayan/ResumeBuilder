...
test("start blank opens editor", async ({ page }) => {
  ...
  await expect(page.getByLabel("Resume preview")).toContainText("Ada Lovelace");
});
...
test("import legacy resume json", async ({ page }) => {
  ...
  await expect(page.getByLabel("Resume preview")).toContainText("Arudchayan Pirabaharan", { timeout: 10000 });
  await expect(page.getByLabel("Resume preview")).toContainText("Employment History");
  await expect(page.getByLabel("Resume preview")).toContainText("Details");
  await expect(page.getByLabel("Resume preview")).toContainText("Profile");
});
